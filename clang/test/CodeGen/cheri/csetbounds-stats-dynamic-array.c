...
// RUN:   -mllvm -collect-csetbounds-stats=csv -cheri-stats-file=%t.csv -S -o /dev/null -O2 -fexperimental-assignment-tracking=disabled
...
int test(int n) {
  ...
  // CSV-NEXT:   0,<unknown>,s,"{{.+}}/csetbounds-stats-dynamic-array.c:[[@LINE-2]]:3","ExpandDYNAMIC_STACKALLOC",""
}

int test2(int n) {
  char* buf = __builtin_alloca(n);
  return do_stuff(buf, n);
  // CSV-NEXT: 0,<unknown>,s,"{{.+}}/csetbounds-stats-dynamic-array.c:[[@LINE-2]]:15","ExpandDYNAMIC_STACKALLOC",""
}
