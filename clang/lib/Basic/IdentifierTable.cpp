...
static KeywordStatus getKeywordStatus(const LangOptions &LangOpts,
  ...
  if (CurStatus == KS_Unknown && LangOpts.CPlusPlus && !LangOpts.CPlusPlus20 &&
      (Flags & KEYCOROUTINES)) {
    return KS_Future;
  }
...
#define COROUTINES_KEYWORD(NAME) .Case(#NAME, KEYCOROUTINES)
#include "clang/Basic/TokenKinds.def"
#undef KEYWORD
      ;
  ...
  if (LangOpts.CPlusPlus) {
    ...
    // Not a CXX20_KEYWORD(), we should re-enable this check after
    // See https://github.com/CTSRD-CHERI/llvm-project/issues/717
    if ((Flags & KEYCOROUTINES) == KEYCOROUTINES)
