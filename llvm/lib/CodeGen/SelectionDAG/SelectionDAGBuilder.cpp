  ...
  if (isa<InlineAsm>(Callee))
    ...
    case Intrinsic::wasm_rethrow: {
      ...
      Ops.push_back(DAG.getTargetConstant(
          Intrinsic::wasm_rethrow, getCurSDLoc(),
...
#include "llvm/IR/VPIntrinsics.def"
  ...
  case Intrinsic::ptrmask: {
    ...
    EVT AddrVT =
        IsCapability ? TLI.getPointerTy(DAG.getDataLayout(), 0) : PtrVT;
    SDValue Result = Ptr;
    ...
    if (IsCapability) {
      Result = DAG.getNode(
          ...
          DAG.getConstant(Intrinsic::cheri_cap_address_set, sdl, AddrVT), Ptr, Result);
    }
...
bool SelectionDAGBuilder::visitMemPCpyCall(const CallInst &I) {
  ...
  SDValue MC =
      DAG.getMemcpy(Root, sdl, Dst, Src, Size, Alignment, false, false,
    ...
    case InlineAsm::isLabel: {
      ...
      if (OpInfo.isMatchingInputConstraint()) {
        ...
        AsmNodeOperands.push_back(DAG.getTargetConstant(
            OpFlag, getCurSDLoc(), TLI.getPointerRangeTy(DAG.getDataLayout())));
