...
; RUN:     opt -opaque-pointers=0 -passes=rel-lookup-table-converter -relocation-model=pic -S | FileCheck %s --check-prefix=RISCV64
; RUN: sed -e 's/addrspace(200)/addrspace(0)/g' -e 's/-A200-P200-G200//g' %s | \
; RUN:     %riscv64_cheri_llc -opaque-pointers=0 -relocation-model=pic | FileCheck %s --check-prefixes=RISCV64-ASM-NOREL
; RUN: sed -e 's/addrspace(200)/addrspace(0)/g' -e 's/-A200-P200-G200//g' %s | opt -opaque-pointers=0 -passes=rel-lookup-table-converter \
...
; RUN: opt -opaque-pointers=0 -o - -passes=rel-lookup-table-converter -relocation-model=pic -S %s | FileCheck %s --check-prefix=PURECAP
; RUN: opt -opaque-pointers=0 -o - -passes=rel-lookup-table-converter -relocation-model=pic -S %s | \
